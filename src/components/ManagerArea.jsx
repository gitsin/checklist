...
export default function ManagerArea({ usuarioAtual, lojaAtual, onBack }) {
  ...
  useEffect(() => {
    buscarDadosGestao();
    const timer = setInterval(() => { setNow(new Date()); buscarDadosGestao(); }, 60000);
  ...
  async function buscarDadosGestao() {
    ...
    subs?.forEach(s => {
        ...
        mapa[s.role_id].push(s.full_name.split(' ')[0]); 
    });
    setMapaResponsaveis(mapa);

    if (idsSubs.length === 0) {
      setTarefasRevisao([]); setTarefasAtrasadas([]); setLoading(false); return;
    }
  ...
  async function confirmarDevolucao() {
    ...
    const { error } = await supabase.from('checklist_items').update({ status: 'RETURNED', manager_notes: feedbackGestor, reviewed_at: new Date().toISOString(), reviewed_by_id: usuarioAtual.id }).eq('id', tarefaParaDevolver.id);
    ...
    <div className="w-full max-w-5xl animate-fade-in mx-auto mt-4 px-2 md:px-4 font-sans text-slate-800 pb-20">
      ...
      <div className="flex items-center justify-between mb-4 bg-white p-3 rounded-xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-3">
            <button onClick={onBack} className="text-slate-400 hover:text-blue-600 p-2"><ArrowLeft size={24}/></button>
            <h2 className="text-lg font-black flex items-center gap-2 text-slate-700"><ClipboardCheck className="text-blue-600" size={24}/> Gestão</h2>
        </div>
      ...
      <div className="flex gap-3 mb-6">
          <button onClick={() => setAbaAtiva('revisao')} className={`flex-1 p-3 rounded-xl border transition-all flex flex-col items-center justify-center gap-1 shadow-sm active:scale-95 ${abaAtiva === 'revisao' ? 'border-blue-500 bg-blue-50 text-blue-800 ring-2 ring-blue-200' : 'border-slate-200 bg-white text-slate-400'}`}>
          ...
          <button onClick={() => setAbaAtiva('atrasadas')} className={`flex-1 p-3 rounded-xl border transition-all flex flex-col items-center justify-center gap-1 shadow-sm active:scale-95 ${abaAtiva === 'atrasadas' ? 'border-red-500 bg-red-50 text-red-800 ring-2 ring-red-200' : 'border-slate-200 bg-white text-slate-400'}`}>
                            ...
                            <div className="pl-3 flex justify-between items-start">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className="flex items-center gap-1 bg-red-100 text-red-700 text-[10px] px-1.5 py-0.5 rounded font-black uppercase">{t.status === 'POSTPONED' ? <CalendarClock size={10}/> : <Clock size={10}/>} {t.horaLimiteEfetiva}</span>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {responsaveis.map((nome, idx) => (
                                            <span key={idx} className="text-xs font-bold text-slate-700 bg-white border border-slate-200 px-1.5 py-0.5 rounded shadow-sm">{nome}</span>
                                        ))}
                                    </div>
                                </div>
                            </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 overflow-y-auto backdrop-blur-sm">
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm relative">
                ...
                <button onClick={() => setModalDevolverOpen(false)} className="flex-1 py-2.5 text-slate-500 font-bold hover:bg-slate-50 rounded-xl text-sm active:scale-95">Cancelar</button>
                <button onClick={confirmarDevolucao} className="flex-1 py-2.5 bg-orange-500 text-white font-bold rounded-xl shadow-md text-sm active:scale-95">Confirmar</button>
                </div>
            </div>
          </div>
        </div>
      ...
      {fotoParaVisualizar && (
        <div className="fixed inset-0 bg-black/90 z-[60] overflow-y-auto backdrop-blur-sm" onClick={() => setFotoParaVisualizar(null)}>
            <div className="flex min-h-full items-center justify-center p-4">
                <div className="relative max-w-full max-h-screen" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => setFotoParaVisualizar(null)} className="absolute -top-12 right-0 text-white p-2"><X size={32} /></button>
                    <img src={fotoParaVisualizar} className="max-w-full max-h-[80vh] rounded-lg border-2 border-white object-contain"/>
                </div>
            </div>
        </div>
      )}
