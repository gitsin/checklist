...
import { 
  ...
  FileUp, AlertCircle, Download, ShieldCheck 
} from "lucide-react";
...
export default function App() {
  ...
  const [filtroManagerFunc, setFiltroManagerFunc] = useState(""); // Novo filtro
  ...
  // ADMIN: COLABORADORES (FIX: QUERY COM JOIN GESTOR)
  async function abrirAdminColaboradores(lojaId = filtroLojaFunc, roleId = filtroRoleFunc, managerId = filtroManagerFunc) {
    ...
    // SINTAXE CORRIGIDA: manager_id referencia o objeto manager na mesma tabela
    ...
    if (error) {
        ...
        console.log("DEBUG - Lista de Colaboradores:", data);
        setListaFuncionariosAdmin(data || []);
    }
  }
  ...
  function abrirModalNovoColaborador() { 
    setNovoFuncNome(""); setNovoFuncLoja(""); setNovoFuncRole(""); setNovoFuncManager(""); 
    setListaGestoresDisponiveis([]); setModalNovoFuncionarioOpen(true); 
  }
  ...
  async function abrirModalEditarColaborador(func) { 
    setEditFuncId(func.id); setEditFuncNome(func.full_name || ""); 
    setEditFuncLoja(func.store_id || ""); setEditFuncRole(func.role_id || ""); 
    setEditFuncManager(func.manager_id || "");
    await carregarGestoresPorLoja(func.store_id);
    setModalEditarFuncionarioOpen(true); 
  }
      ...
      <div className="min-h-screen bg-slate-800 text-white p-8 font-sans">
        <div className="max-w-6xl mx-auto">
          ...
          {adminScreen === 'cargos' && (
            <div className="animate-fade-in">
              <button onClick={() => setAdminScreen('menu')} className="mb-6 flex items-center gap-2 text-slate-400 hover:text-white"><ArrowLeft /> Voltar ao Menu</button>
              <div className="bg-slate-700 p-6 rounded-xl mb-8 border border-slate-600"><h3 className="text-xl font-bold mb-4">Adicionar Novo Cargo</h3><div className="flex gap-4"><input type="text" placeholder="Nome (ex: Cozinheiro)" className="flex-1 p-3 rounded bg-slate-800 border border-slate-600 text-white outline-none focus:border-blue-500" value={novoCargoNome} onChange={(e) => setNovoCargoNome(e.target.value)} /><input type="number" placeholder="Nível" className="w-32 p-3 rounded bg-slate-800 border border-slate-600 text-white outline-none focus:border-blue-500" value={novoCargoNivel} onChange={(e) => setNovoCargoNivel(e.target.value)} /><button onClick={criarCargo} className="bg-blue-600 px-6 py-3 rounded hover:bg-blue-50 font-bold flex items-center gap-2"><Plus size={20}/> Criar</button></div></div>
              <div className="grid gap-4 md:grid-cols-2">{roles.map(cargo => (<div key={cargo.id} className={`p-4 rounded-lg flex justify-between items-center shadow-sm border-l-8 ${cargo.active ? 'bg-white text-slate-800 border-green-500' : 'bg-slate-300 text-slate-500 border-slate-500'}`}><div className="text-slate-800"><span className="font-bold text-lg block">{cargo.name}</span><span className="text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded mr-2">Nível: {cargo.access_level}</span></div><div className="flex gap-2 items-center"><button onClick={() => toggleStatusCargo(cargo)} className="flex items-center gap-2 px-3 py-2 rounded font-bold text-sm transition-colors" style={{ backgroundColor: cargo.active ? '#dcfce7' : '#f1f5f9', color: cargo.active ? '#166534' : '#64748b' }}>{cargo.active ? <ToggleRight size={24}/> : <ToggleLeft size={24}/>}</button><button onClick={() => abrirModalEditarCargo(cargo)} className="text-blue-600 hover:text-blue-800 p-2 hover:bg-blue-50 rounded bg-white border border-blue-100"><Pencil size={20} /></button></div></div>))}</div>
            </div>
          )}

          {/* TELA COLABORADORES (3 FILTROS + FIX GESTOR NO CARD) */}
          {adminScreen === 'colaboradores' && (
            <div className="animate-fade-in">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={abrirModalNovoColaborador} className="bg-blue-600 px-4 py-2 rounded font-bold hover:bg-blue-500 flex items-center gap-2 transition-all shadow-md"><Plus size={20} /> Novo Colaborador</button>
              </div>
              
              <div className="bg-slate-700 p-4 rounded-xl mb-6 flex flex-col md:flex-row gap-4 items-end border border-slate-600">
                <div className="flex-1 w-full"><label className="text-[10px] text-slate-400 uppercase font-bold block mb-1">Loja</label><select className="w-full p-2 rounded bg-slate-800 border border-slate-500 text-white outline-none" value={filtroLojaFunc} onChange={(e) => { setFiltroLojaFunc(e.target.value); abrirAdminColaboradores(e.target.value, filtroRoleFunc, filtroManagerFunc); }}><option value="">Todas as lojas</option>{lojas.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}</select></div>
                <div className="flex-1 w-full"><label className="text-[10px] text-slate-400 uppercase font-bold block mb-1">Cargo</label><select className="w-full p-2 rounded bg-slate-800 border border-slate-500 text-white outline-none" value={filtroRoleFunc} onChange={(e) => { setFiltroRoleFunc(e.target.value); abrirAdminColaboradores(filtroLojaFunc, e.target.value, filtroManagerFunc); }}><option value="">Todos os cargos</option>{roles.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}</select></div>
                <div className="flex-1 w-full"><label className="text-[10px] text-slate-400 uppercase font-bold block mb-1">Gestor</label><select className="w-full p-2 rounded bg-slate-800 border border-slate-500 text-white outline-none" value={filtroManagerFunc} onChange={(e) => { setFiltroManagerFunc(e.target.value); abrirAdminColaboradores(filtroLojaFunc, filtroRoleFunc, e.target.value); }}>
                  <option value="">Todos os gestores</option>
                  {[...new Set(listaFuncionariosAdmin.filter(f => f.manager_id).map(f => f.manager_id))].map(mid => {
                      const g = listaFuncionariosAdmin.find(x => x.id === mid) || (listaFuncionariosAdmin.find(x => x.manager_id === mid)?.manager);
                      return g ? <option key={mid} value={mid}>{g.full_name}</option> : null;
                  })}
                </select></div>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {listaFuncionariosAdmin.map(func => (
                  <div key={func.id} className={`p-4 rounded-lg flex justify-between items-center shadow-sm border-l-8 ${func.active ? 'bg-white text-slate-800 border-green-500' : 'bg-slate-300 text-slate-500 border-slate-500'}`}>
                    <div>
                      <span className="font-bold block text-lg">{func.full_name}</span>
                      <span className="text-xs text-slate-500 uppercase font-bold">{func.stores?.name} • {func.roles?.name}</span>
                      {func.manager?.full_name && (
                          <span className="text-xs text-blue-600 flex items-center gap-1 mt-1 font-bold italic bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100 w-fit">
                              <ShieldCheck size={12}/> Gestor: {func.manager.full_name}
                          </span>
                      )}
                    </div>
                    <div className="flex gap-2 items-center">
                      <button onClick={() => toggleStatusFuncionario(func)} className="px-2 py-1 rounded">{func.active ? <ToggleRight className="text-green-600" size={32}/> : <ToggleLeft size={32}/>}</button>
                      <button onClick={() => abrirModalEditarColaborador(func)} className="text-blue-600 p-2 hover:bg-blue-50 rounded-full transition-colors"><Pencil size={20} /></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="animate-fade-in">
              <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div className="flex gap-4">
                  <button onClick={() => setModalImportarOpen(true)} className="bg-slate-600 px-4 py-2 rounded font-bold hover:bg-slate-50 flex items-center gap-2 shadow-lg transition-all"><FileUp size={20} /> Importar CSV</button>
                  <button onClick={gerarRotinaDoDia} disabled={gerandoRotina} className="bg-green-600 px-4 py-2 rounded font-bold flex items-center gap-2 shadow-lg disabled:opacity-50 disabled:cursor-wait"><PlayCircle size={20} /> Gerar Rotina</button>
                  <button onClick={abrirModalNovaTarefa} className="bg-purple-600 px-4 py-2 rounded font-bold flex items-center gap-2 shadow-lg hover:bg-purple-500 transition-all"><Plus size={20} /> Nova Tarefa</button>
                </div>
              </div>
          ...
          {adminScreen === 'relatorios' && (
            <div className="animate-fade-in">
              ...
              <div className="bg-slate-700 p-6 rounded-xl mb-8 border border-slate-600">
                ...
                <select className="w-full p-3 rounded bg-slate-800 border border-slate-500 text-white outline-none text-lg" value={filtroLojaRelatorio} onChange={(e) => { setFiltroLojaRelatorio(e.target.value); buscarDashboardOperacional(e.target.value); }}><option value="">-- Selecione uma loja --</option>{lojas.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}</select>
              </div>
              {filtroLojaRelatorio && statusContagem && !loading && (
                <div className="bg-white p-6 rounded-xl text-slate-800 shadow-xl">
                  ...
                  <div className="mt-10 pt-4 border-t border-slate-200"><h4 className="text-xl font-bold mb-4 flex items-center gap-2 text-teal-700"><BarChart3 size={20} /> Eficiência por Cargo (Hoje)</h4><div className="space-y-4">{eficienciaPorCargo.map((data) => (<div key={data.roleName} className="p-3 bg-slate-50 rounded-lg border border-slate-200"><div className="flex justify-between items-center mb-1"><span className="font-semibold text-slate-700">{data.roleName}</span><span className="font-bold text-lg" style={{ color: data.percentCompleted >= 90 ? '#059669' : '#f59e0b' }}>{data.percentCompleted.toFixed(0)}%</span></div><div className="w-full bg-slate-200 rounded-full h-2.5"><div className="h-2.5 rounded-full" style={{ width: `${data.percentCompleted}%`, backgroundColor: data.percentCompleted >= 90 ? '#10b981' : '#f59e0b' }}></div></div></div>))}</div></div>
                </div>
              )}
            </div>
          )}
        ...
        {modalNovoFuncionarioOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><Plus className="text-blue-600"/> Novo Colaborador</h3><button onClick={() => setModalNovoFuncionarioOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="space-y-4">
          <div><label className="block text-sm font-bold text-slate-600 mb-1">Nome Completo</label><input type="text" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-blue-500" placeholder="Ex: João da Silva" value={novoFuncNome} onChange={(e) => setNovoFuncNome(e.target.value)} /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-bold text-slate-600 mb-1">Loja</label><select className="w-full border border-slate-300 rounded-lg p-3 bg-white" value={novoFuncLoja} onChange={(e) => { setNovoFuncLoja(e.target.value); carregarGestoresPorLoja(e.target.value); }}><option value="">Selecione...</option>{lojas.map(loja => (<option key={loja.id} value={loja.id}>{loja.name}</option>))}</select></div>
          ...
          <div><label className="block text-sm font-bold text-slate-600 mb-1">Gestor Responsável (Opcional)</label><select className="w-full border border-slate-300 rounded-lg p-3 bg-white" value={novoFuncManager} onChange={(e) => setNovoFuncManager(e.target.value)} disabled={!novoFuncLoja}><option value="">Nenhum / Diretor</option>{listaGestoresDisponiveis.map(g => (<option key={g.id} value={g.id}>{g.full_name}</option>))}</select></div>
        </div><div className="flex gap-3 mt-8"><button onClick={() => setModalNovoFuncionarioOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Cancelar</button><button onClick={salvarNovoColaborador} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-md flex justify-center items-center gap-2 transition-all"><Save size={18} /> Criar</button></div></div></div>)}
        ...
        {modalEditarFuncionarioOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><Pencil className="text-blue-500"/> Editar Colaborador</h3><button onClick={() => setModalEditarFuncionarioOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="space-y-4">
          <div><label className="block text-sm font-bold text-slate-600 mb-1">Nome Completo</label><input type="text" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-blue-500" value={editFuncNome} onChange={(e) => setEditFuncNome(e.target.value)} /></div>
          <div className="grid grid-cols-2 gap-4">
        ...
        </div><div className="flex gap-3 mt-8"><button onClick={() => setModalEditarFuncionarioOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Cancelar</button><button onClick={salvarEdicaoColaborador} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-md flex justify-center items-center gap-2 transition-all"><Save size={18} /> Salvar</button></div></div></div>)}
        ...
        {modalNovaTarefaOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><Plus className="text-purple-600"/> Nova Tarefa</h3><button onClick={() => setModalNovaTarefaOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-bold text-slate-600 mb-1">Loja</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={novaTarefaLoja} onChange={(e) => handleLojaChangeNovaTarefa(e.target.value)}><option value="">Selecione a Loja...</option>{lojas.filter(l => l.active).map(l => (<option key={l.id} value={l.id}>{l.name}</option>))}</select></div>
            <div><label className="block text-sm font-bold text-slate-600 mb-1">Cargo Responsável</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={novaTarefaRole} onChange={(e) => setNovaTarefaRole(e.target.value)} disabled={!novaTarefaLoja || rolesDaLoja.length === 0}><option value="">Selecione o Cargo...</option>{rolesDaLoja.map(r => (<option key={r.id} value={r.id}>{r.name}</option>))}</select></div>
          </div>
          ...
          <div><label className="block text-sm font-bold text-slate-600 mb-1">Instruções de Execução</label><textarea className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 h-24" placeholder="Detalhes..." value={novaTarefaDesc} onChange={(e) => setNovaTarefaDesc(e.target.value)} /></div>
          <div><label className="block text-sm font-bold text-slate-600 mb-1">Frequência</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={novaTarefaFreq} onChange={(e) => setNovaTarefaFreq(e.target.value)}><option value="daily">Diária</option><option value="weekly">Semanal</option><option value="monthly">Mensal</option></select></div>
          <div className="grid grid-cols-2 gap-4 items-center pt-2">
            <div><label className="block text-sm font-bold text-slate-600 mb-1">Horário Limite</label><input type="time" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500" value={novaTarefaDueTime} onChange={(e) => setNovaTarefaDueTime(e.target.value)} /></div>
            <div className="flex items-center gap-3 pt-7"><input type="checkbox" id="nova-foto" className="h-5 w-5 rounded border-slate-400 text-purple-600 focus:ring-purple-500" checked={novaTarefaPhotoEvidence} onChange={(e) => setNovaTarefaPhotoEvidence(e.target.checked)} /><label htmlFor="nova-foto" className="font-bold text-slate-600 text-sm">Exige Foto</label></div>
          </div>
        </div><div className="flex gap-3 mt-8"><button onClick={() => setModalNovaTarefaOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Cancelar</button><button onClick={salvarNovaTarefa} className="flex-1 py-3 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 shadow-md flex justify-center items-center gap-2 transition-all"><Save size={18} /> Salvar</button></div></div></div>)}
        ...
        {modalEditarTarefaOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><Pencil className="text-purple-600"/> Editar Tarefa</h3><button onClick={() => setModalEditarTarefaOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-bold text-slate-600 mb-1">Loja</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={editTarefaLoja} onChange={(e) => handleLojaChangeEditarTarefa(e.target.value)}><option value="">Selecione a Loja...</option>{lojas.filter(l => l.active).map(l => (<option key={l.id} value={l.id}>{l.name}</option>))}</select></div>
            <div><label className="block text-sm font-bold text-slate-600 mb-1">Cargo Responsável</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={editTarefaRole} onChange={(e) => setEditTarefaRole(e.target.value)} disabled={!editTarefaLoja || rolesDaLojaEmEdicao.length === 0}><option value="">Selecione o Cargo...</option>{rolesDaLojaEmEdicao.map(r => (<option key={r.id} value={r.id}>{r.name}</option>))}</select></div>
          </div>
          ...
          <div><label className="block text-sm font-bold text-slate-600 mb-1">Instruções</label><textarea className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 h-24" value={editTarefaDesc} onChange={(e) => setEditTarefaDesc(e.target.value)} /></div>
          <div><label className="block text-sm font-bold text-slate-600 mb-1">Frequência</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={editTarefaFreq} onChange={(e) => setEditTarefaFreq(e.target.value)}><option value="daily">Diária</option><option value="weekly">Semanal</option><option value="monthly">Mensal</option></select></div>
          <div className="grid grid-cols-2 gap-4 items-center pt-2">
            <div><label className="block text-sm font-bold text-slate-600 mb-1">Horário Limite</label><input type="time" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500" value={editTarefaDueTime} onChange={(e) => setEditTarefaDueTime(e.target.value)} /></div>
            <div className="flex items-center gap-3 pt-7"><input type="checkbox" id="edit-foto" className="h-5 w-5 rounded border-slate-400 text-purple-600 focus:ring-purple-500" checked={editTarefaPhotoEvidence} onChange={(e) => setEditTarefaPhotoEvidence(e.target.checked)} /><label htmlFor="edit-foto" className="font-bold text-slate-600 text-sm">Exige Foto</label></div>
          </div>
        </div><div className="flex gap-3 mt-8"><button onClick={() => setModalEditarTarefaOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Cancelar</button><button onClick={salvarEdicaoTarefa} className="flex-1 py-3 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 shadow-md flex justify-center items-center gap-2 transition-all"><Save size={18} /> Salvar</button></div></div></div>)}
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg text-slate-800 animate-fade-in">
              ...
              <div className="bg-blue-50 p-4 rounded-lg mb-6 border border-blue-100">
                <p className="text-sm font-bold text-blue-800 mb-2">Modelo de Importação:</p>
                <button onClick={baixarExemploCSV} className="flex items-center gap-2 text-xs bg-blue-600 text-white px-3 py-2 rounded-lg font-bold hover:bg-blue-700 transition-all shadow-md"><Download size={14} /> Baixar Modelo CSV</button>
              </div>
              <input type="file" accept=".csv" onChange={processarImportacaoCSV} disabled={importando} className="w-full border-2 border-dashed border-slate-300 rounded-xl p-8 text-center" />
