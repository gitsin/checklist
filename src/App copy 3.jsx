...
import { 
  ...
  FileUp, AlertCircle, Download, ShieldCheck, AlertTriangle, RefreshCcw 
} from "lucide-react";
...
export default function App() {
  ...
  // Admin: Colaboradores (Hierarquia Fase 1)
  ...
  const [filtroManagerFunc, setFiltroManagerFunc] = useState(""); 
  ...
  const [novoFuncManager, setNovoFuncManager] = useState(""); 
  ...
  const [editFuncManager, setEditFuncManager] = useState(""); 
  const [listaGestoresDisponiveis, setListaGestoresDisponiveis] = useState([]); 
  ...
  async function carregarGestoresPorLoja(lojaId) {
    if (!lojaId) { setListaGestoresDisponiveis([]); return; }
    const { data } = await supabase.from("employee").select("id, full_name").eq("store_id", lojaId).eq("active", true);
    setListaGestoresDisponiveis(data || []);
  }
  ...
  async function buscarTarefasDoDia(storeId, roleId) {
    ...
    const { data, error } = await supabase
      ...
      .eq("store_id", storeId).eq("scheduled_date", hoje).eq("template.role_id", roleParaBuscar);
    
    if (error) {
      alert("Erro: " + error.message);
    } else {
      // Ordenação manual: RETURNED no topo, depois PENDING, etc.
      const ordens = { 'RETURNED': 1, 'PENDING': 2, 'WAITING_APPROVAL': 3, 'POSTPONED': 4, 'COMPLETED': 5, 'CANCELED': 6 };
      const ordenado = (data || []).sort((a, b) => ordens[a.status] - ordens[b.status]);
      setTarefas(ordenado);
    }
    setLoading(false);
  }
  ...
  async function handleConcluir(e, tarefa) {
    ...
    // LÓGICA FASE 2: Fluxo de Aprovação
    if (tarefa.status === 'PENDING' || tarefa.status === 'RETURNED') {
      // Se tem gestor, vai para aprovação. Se não tem, conclui direto.
      novoStatus = usuarioAtual.manager_id ? 'WAITING_APPROVAL' : 'COMPLETED';
    } else {
      // Se já estava concluído ou aguardando, volta para pendente
      novoStatus = 'PENDING';
    }

    await supabase.from("checklist_items").update({ 
        status: novoStatus, 
        completed_by_employee_id: (novoStatus === 'COMPLETED' || novoStatus === 'WAITING_APPROVAL') ? usuarioAtual.id : null,
        completed_at: (novoStatus === 'COMPLETED' || novoStatus === 'WAITING_APPROVAL') ? new Date().toISOString() : null,
  ...
  function sairDoAdmin() { 
    ...
    setFiltroLojaFunc(""); setFiltroRoleFunc(""); setFiltroManagerFunc("");
  ...
  // ADMIN: COLABORADORES
  async function abrirAdminColaboradores(lojaId = filtroLojaFunc, roleId = filtroRoleFunc, managerId = filtroManagerFunc) {
    setAdminScreen('colaboradores');
    let query = supabase.from("employee").select(`*, stores (name), roles (name), manager:manager_id (full_name)`).order('full_name');
  ...
  async function salvarNovoColaborador() {
    const { error } = await supabase.from("employee").insert({ 
      full_name: novoFuncNome, store_id: novoFuncLoja, role_id: novoFuncRole, manager_id: novoFuncManager || null, active: true 
    });
  ...
  async function salvarEdicaoColaborador() {
    const { error } = await supabase.from("employee").update({ 
      full_name: editFuncNome, store_id: editFuncLoja, role_id: editFuncRole, manager_id: editFuncManager || null
    }).eq("id", editFuncId);
    if (error) alert(error.message); else { setModalEditarFuncionarioOpen(false); abrirAdminColaboradores(); }
  }
  ...
  async function salvarNovaTarefa() {
    ...
    if (error) alert(error.message); else { setModalNovaTarefaOpen(false); buscarTemplatesFiltrados(novaTarefaLoja, novaTarefaRole); }
  }
  ...
  async function salvarEdicaoTarefa() {
    ...
    if (error) alert(error.message); else { setModalEditarTarefaOpen(false); buscarTemplatesFiltrados(filtroLojaTemplate, filtroRoleTemplate); }
  }
  ...
  function calcularStatusContagem(items) {
    const counts = { COMPLETED: 0, WAITING_APPROVAL: 0, RETURNED: 0, PENDING: 0, POSTPONED: 0, CANCELED: 0, TOTAL: items.length };
  ...
  if (view === 'admin') {
    return (
      <div className="min-h-screen bg-slate-800 text-white p-8 font-sans">
          ...
          {adminScreen === 'colaboradores' && (
            <div className="animate-fade-in">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={abrirModalNovoColaborador} className="bg-blue-600 px-4 py-2 rounded font-bold flex items-center gap-2"><Plus size={20} /> Novo Colaborador</button>
              </div>
              <div className="bg-slate-700 p-4 rounded-xl mb-6 flex flex-col md:flex-row gap-4 border border-slate-600">
                <div className="flex-1 w-full"><label className="text-[10px] uppercase font-bold text-slate-400 mb-1 block">Loja</label><select className="w-full p-2 rounded bg-slate-800" value={filtroLojaFunc} onChange={(e) => { setFiltroLojaFunc(e.target.value); abrirAdminColaboradores(e.target.value, filtroRoleFunc, filtroManagerFunc); }}><option value="">Todas as Lojas</option>{lojas.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}</select></div>
                <div className="flex-1 w-full"><label className="text-[10px] uppercase font-bold text-slate-400 mb-1 block">Cargo</label><select className="w-full p-2 rounded bg-slate-800" value={filtroRoleFunc} onChange={(e) => { setFiltroRoleFunc(e.target.value); abrirAdminColaboradores(filtroLojaFunc, e.target.value, filtroManagerFunc); }}><option value="">Todos os Cargos</option>{roles.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}</select></div>
                <div className="flex-1 w-full"><label className="text-[10px] uppercase font-bold text-slate-400 mb-1 block">Gestor</label><select className="w-full p-2 rounded bg-slate-800" value={filtroManagerFunc} onChange={(e) => { setFiltroManagerFunc(e.target.value); abrirAdminColaboradores(filtroLojaFunc, filtroRoleFunc, e.target.value); }}><option value="">Todos os Gestores</option>{[...new Set(listaFuncionariosAdmin.filter(f => f.manager_id).map(f => f.manager_id))].map(mid => {const g = listaFuncionariosAdmin.find(x => x.id === mid) || listaFuncionariosAdmin.find(x => x.manager_id === mid)?.manager; return g ? <option key={mid} value={mid}>{g.full_name}</option> : null; })}</select></div>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                {listaFuncionariosAdmin.map(func => (
                  <div key={func.id} className={`p-4 rounded-lg flex justify-between items-center border-l-8 ${func.active ? 'bg-white text-slate-800 border-green-500' : 'bg-slate-300 text-slate-500 border-slate-500'}`}>
                    <div><span className="font-bold block text-lg">{func.full_name}</span><span className="text-xs text-slate-500 font-bold">{func.stores?.name} • {func.roles?.name}</span>{func.manager?.full_name && (<span className="text-xs text-blue-600 flex items-center gap-1 mt-1 font-bold italic bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100 w-fit"><ShieldCheck size={12}/> Gestor: {func.manager.full_name}</span>)}</div>
                    <div className="flex gap-2"><button onClick={() => toggleStatusFuncionario(func)}>{func.active ? <ToggleRight className="text-green-600" size={32}/> : <ToggleLeft size={32}/>}</button><button onClick={() => abrirModalEditarColaborador(func)} className="text-blue-600 p-2"><Pencil size={20} /></button></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="animate-fade-in">
              ...
              {statusContagem && (
                <div className="bg-white p-6 rounded-xl text-slate-800">
                  <h3 className="text-xl font-bold mb-6">Resumo Operacional: {statusContagem.PERCENT_COMPLETED.toFixed(0)}% Concluído</h3>
                  <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                    <div className="bg-slate-100 p-4 rounded-lg text-center font-bold uppercase"><span className="text-xs block">Total</span>{statusContagem.TOTAL}</div>
                    <div className="bg-green-100 p-4 rounded-lg text-center font-bold uppercase text-green-700"><span className="text-xs block">Feito</span>{statusContagem.COMPLETED}</div>
                    <div className="bg-blue-100 p-4 rounded-lg text-center font-bold uppercase text-blue-700"><span className="text-xs block">Revisão</span>{statusContagem.WAITING_APPROVAL}</div>
                    <div className="bg-orange-100 p-4 rounded-lg text-center font-bold uppercase text-orange-700"><span className="text-xs block">Devolvido</span>{statusContagem.RETURNED}</div>
                    <div className="bg-red-100 p-4 rounded-lg text-center font-bold uppercase text-red-700"><span className="text-xs block">Cancelado</span>{statusContagem.CANCELED}</div>
                  </div>
                </div>
              )}
            </div>
        ...
        {modalNovoFuncionarioOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><Plus className="text-blue-600"/> Novo Colaborador</h3><button onClick={() => setModalNovoFuncionarioOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="space-y-4"><div><label className="block text-sm font-bold text-slate-600 mb-1">Nome Completo</label><input type="text" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-blue-500" placeholder="Ex: João da Silva" value={novoFuncNome} onChange={(e) => setNovoFuncNome(e.target.value)} /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-slate-600 mb-1">Loja</label><select className="w-full border border-slate-300 rounded-lg p-3 bg-white" value={novoFuncLoja} onChange={(e) => { setNovoFuncLoja(e.target.value); carregarGestoresPorLoja(e.target.value); }}><option value="">Selecione...</option>{lojas.map(loja => (<option key={loja.id} value={loja.id}>{loja.name}</option>))}</select></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Cargo</label><select className="w-full border border-slate-300 rounded-lg p-3 bg-white" value={novoFuncRole} onChange={(e) => setNovoFuncRole(e.target.value)}><option value="">Selecione...</option>{roles.map(role => (<option key={role.id} value={role.id}>{role.name}</option>))}</select></div></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Gestor Responsável (Opcional)</label><select className="w-full border border-slate-300 rounded-lg p-3 bg-white" value={novoFuncManager} onChange={(e) => setNovoFuncManager(e.target.value)} disabled={!novoFuncLoja}><option value="">Nenhum / Diretor</option>{listaGestoresDisponiveis.map(g => (<option key={g.id} value={g.id}>{g.full_name}</option>))}</select></div></div><div className="flex gap-3 mt-8"><button onClick={() => setModalNovoFuncionarioOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg">Cancelar</button><button onClick={salvarNovoColaborador} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-md flex justify-center items-center gap-2"><Save size={18} /> Criar</button></div></div></div>)}
        {modalEditarFuncionarioOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><Pencil className="text-blue-500"/> Editar Colaborador</h3><button onClick={() => setModalEditarFuncionarioOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="space-y-4"><div><label className="block text-sm font-bold text-slate-600 mb-1">Nome Completo</label><input type="text" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-blue-500" value={editFuncNome} onChange={(e) => setEditFuncNome(e.target.value)} /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-slate-600 mb-1">Loja</label><select className="w-full border border-slate-300 rounded-lg p-3 bg-white" value={editFuncLoja} onChange={(e) => { setEditFuncLoja(e.target.value); carregarGestoresPorLoja(e.target.value); }}><option value="">Selecione...</option>{lojas.map(loja => (<option key={loja.id} value={loja.id}>{loja.name}</option>))}</select></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Cargo</label><select className="w-full border border-slate-300 rounded-lg p-3 bg-white" value={editFuncRole} onChange={(e) => setEditFuncRole(e.target.value)}><option value="">Selecione...</option>{roles.map(role => (<option key={role.id} value={role.id}>{role.name}</option>))}</select></div></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Gestor Responsável</label><select className="w-full border border-slate-300 rounded-lg p-3 bg-white outline-none focus:border-blue-500" value={editFuncManager} onChange={(e) => setEditFuncManager(e.target.value)} disabled={!editFuncLoja}><option value="">Nenhum / Diretor Geral</option>{listaGestoresDisponiveis.filter(g => g.id !== editFuncId).map(g => (<option key={g.id} value={g.id}>{g.full_name}</option>))}</select></div></div><div className="flex gap-3 mt-8"><button onClick={() => setModalEditarFuncionarioOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Cancelar</button><button onClick={salvarEdicaoColaborador} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-md flex justify-center items-center gap-2"><Save size={18} /> Salvar</button></div></div></div>)}

        {/* MODAL NOVA TAREFA */}
        {modalNovaTarefaOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><Plus className="text-purple-600"/> Nova Tarefa</h3><button onClick={() => setModalNovaTarefaOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="space-y-4"><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-slate-600 mb-1">Loja</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={novaTarefaLoja} onChange={(e) => handleLojaChangeNovaTarefa(e.target.value)}><option value="">Selecione a Loja...</option>{lojas.filter(l => l.active).map(l => (<option key={l.id} value={l.id}>{l.name}</option>))}</select></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Cargo</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={novaTarefaRole} onChange={(e) => setNovaTarefaRole(e.target.value)} disabled={!novaTarefaLoja || rolesDaLoja.length === 0}><option value="">Selecione o Cargo...</option>{rolesDaLoja.map(r => (<option key={r.id} value={r.id}>{r.name}</option>))}</select></div></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Título</label><input type="text" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500" placeholder="Ex: Verificar temperatura freezers" value={novaTarefaTitulo} onChange={(e) => setNovaTarefaTitulo(e.target.value)} /></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Descrição</label><textarea className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 h-24" placeholder="Detalhes..." value={novaTarefaDesc} onChange={(e) => setNovaTarefaDesc(e.target.value)} /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-slate-600 mb-1">Frequência</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={novaTarefaFreq} onChange={(e) => setNovaTarefaFreq(e.target.value)}><option value="daily">Diária</option><option value="weekly">Semanal</option><option value="monthly">Mensal</option></select></div><div className="grid grid-cols-2 gap-4 items-center pt-2"><div><label className="block text-sm font-bold text-slate-600 mb-1">Hora Limite</label><input type="time" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500" value={novaTarefaDueTime} onChange={(e) => setNovaTarefaDueTime(e.target.value)} /></div><div className="flex items-center gap-3 pt-7"><input type="checkbox" id="nova-foto" className="h-5 w-5 rounded border-slate-400 text-purple-600 focus:ring-purple-500" checked={novaTarefaPhotoEvidence} onChange={(e) => setNovaTarefaPhotoEvidence(e.target.checked)} /><label htmlFor="nova-foto" className="font-bold text-slate-600 text-sm">Exige Foto</label></div></div></div></div><div className="flex gap-3 mt-8"><button onClick={() => setModalNovaTarefaOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Cancelar</button><button onClick={salvarNovaTarefa} className="flex-1 py-3 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 shadow-md flex justify-center items-center gap-2 transition-all"><Save size={18} /> Salvar</button></div></div></div>)}

        {/* MODAL EDITAR TAREFA */}
        {modalEditarTarefaOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><Pencil className="text-purple-600"/> Editar Tarefa</h3><button onClick={() => setModalEditarTarefaOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="space-y-4"><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-slate-600 mb-1">Loja</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={editTarefaLoja} onChange={(e) => handleLojaChangeEditarTarefa(e.target.value)}><option value="">Selecione a Loja...</option>{lojas.filter(l => l.active).map(l => (<option key={l.id} value={l.id}>{l.name}</option>))}</select></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Cargo</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={editTarefaRole} onChange={(e) => setEditTarefaRole(e.target.value)} disabled={!editTarefaLoja || rolesDaLojaEmEdicao.length === 0}><option value="">Selecione o Cargo...</option>{rolesDaLojaEmEdicao.map(r => (<option key={r.id} value={r.id}>{r.name}</option>))}</select></div></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Título</label><input type="text" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500" value={editTarefaTitulo} onChange={(e) => setEditTarefaTitulo(e.target.value)} /></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Descrição</label><textarea className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 h-24" value={editTarefaDesc} onChange={(e) => setEditTarefaDesc(e.target.value)} /></div><div><label className="block text-sm font-bold text-slate-600 mb-1">Frequência</label><select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500 bg-white" value={editTarefaFreq} onChange={(e) => setEditTarefaFreq(e.target.value)}><option value="daily">Diária</option><option value="weekly">Semanal</option><option value="monthly">Mensal</option></select></div><div className="grid grid-cols-2 gap-4 items-center pt-2"><div><label className="block text-sm font-bold text-slate-600 mb-1">Horário Limite</label><input type="time" className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-purple-500" value={editTarefaDueTime} onChange={(e) => setEditTarefaDueTime(e.target.value)} /></div><div className="flex items-center gap-3 pt-7"><input type="checkbox" id="edit-foto" className="h-5 w-5 rounded border-slate-400 text-purple-600 focus:ring-purple-500" checked={editTarefaPhotoEvidence} onChange={(e) => setEditTarefaPhotoEvidence(e.target.checked)} /><label htmlFor="edit-foto" className="font-bold text-slate-600 text-sm">Exige Foto</label></div></div></div><div className="flex gap-3 mt-8"><button onClick={() => setModalEditarTarefaOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Cancelar</button><button onClick={salvarEdicaoTarefa} className="flex-1 py-3 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 shadow-md flex justify-center items-center gap-2 transition-all"><Save size={18} /> Salvar</button></div></div></div>)}

        {/* MODAL IMPORTAR CSV */}
        {modalImportarOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg text-slate-800 animate-fade-in"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold flex items-center gap-2"><FileUp className="text-blue-600"/> Importar Tarefas</h3><button onClick={() => {setModalImportarOpen(false); setLogImportacao([]);}} className="text-slate-400 hover:text-slate-600"><X /></button></div><div className="bg-blue-50 p-4 rounded-lg mb-6 border border-blue-100"><p className="text-sm font-bold text-blue-800 mb-2">Modelo de Importação:</p><button onClick={baixarExemploCSV} className="flex items-center gap-2 text-xs bg-blue-600 text-white px-3 py-2 rounded-lg font-bold hover:bg-blue-700 transition-all shadow-md"><Download size={14} /> Baixar Modelo CSV</button></div><input type="file" accept=".csv" onChange={processarImportacaoCSV} disabled={importando} className="w-full border-2 border-dashed border-slate-300 rounded-xl p-8 text-center" />{logImportacao.length > 0 && (<div className="mt-6 max-h-40 overflow-y-auto bg-slate-900 text-slate-300 p-4 rounded-lg text-xs font-mono">{logImportacao.map((log, i) => <div key={i}>{log}</div>)}</div>)}</div></div>)}

        {/* MODAIS QUIOSQUE */}
        {modalAdiarOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md text-slate-800 animate-fade-in"><h3 className="text-xl font-bold text-slate-800 mb-4">Adiar Tarefa</h3><input type="datetime-local" className="w-full border-2 border-slate-200 rounded-lg p-3 text-lg mb-6 outline-none focus:border-amber-500" value={novaDataPrazo} onChange={(e) => setNovaDataPrazo(e.target.value)} /><div className="flex gap-3"><button onClick={() => setModalAdiarOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Voltar</button><button onClick={confirmarAdiamento} className="flex-1 py-3 bg-amber-500 text-white font-bold rounded-lg hover:bg-amber-600 shadow-md transition-all">Confirmar</button></div></div></div>)}
        {modalCancelarOpen && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md text-slate-800 animate-fade-in"><h3 className="text-xl font-bold text-slate-800 mb-4">Cancelar Tarefa</h3><textarea className="w-full border-2 border-slate-200 rounded-lg p-3 mb-6 min-h-[100px] outline-none focus:border-red-500" placeholder="Motivo..." value={justificativa} onChange={(e) => setJustificativa(e.target.value)} /><div className="flex gap-3"><button onClick={() => setModalCancelarOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-lg transition-colors">Voltar</button><button onClick={confirmarCancelamento} className="flex-1 py-3 bg-red-500 text-white font-bold rounded-lg hover:bg-red-600 shadow-md transition-all">Confirmar</button></div></div></div>)}
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col items-center py-8 px-4 font-sans text-slate-800">
      ...
      {usuarioAtual && (
        <div className="w-full max-w-3xl pb-20"><div className="bg-blue-600 text-white p-6 rounded-t-3xl shadow-lg flex items-center justify-between sticky top-0 z-10"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center text-xl font-bold">{usuarioAtual.full_name.charAt(0)}</div><div><h2 className="text-xl font-bold">{usuarioAtual.full_name}</h2><p className="text-blue-100 text-sm">Operando: {lojaAtual.name}</p></div></div><button onClick={voltarParaFuncionarios} className="bg-white/10 hover:bg-white/20 py-2 px-4 rounded-lg text-sm transition-colors">Voltar</button></div>
          <div className="bg-white rounded-b-3xl shadow-lg p-4 min-h-[500px]">{loading ? <div className="text-center py-10 text-slate-400">Carregando...</div> : tarefas.length === 0 ? <div className="text-center py-10 bg-slate-50 rounded-xl border border-dashed border-slate-200"><p className="text-slate-500 font-medium">Tudo limpo por hoje!</p></div> : (<div className="space-y-4">{tarefas.map((tarefa) => { 
              const isCompleted = tarefa.status === 'COMPLETED'; 
              const isPostponed = tarefa.status === 'POSTPONED'; 
              const isCanceled = tarefa.status === 'CANCELED';
              const isWaiting = tarefa.status === 'WAITING_APPROVAL';
              const isReturned = tarefa.status === 'RETURNED';

              let cardClass = "border-slate-100 bg-white";
              if (isCompleted) cardClass = "border-green-200 bg-green-50 opacity-75";
              if (isPostponed) cardClass = "border-amber-200 bg-amber-50";
              if (isCanceled) cardClass = "border-red-200 bg-red-50 opacity-60";
              if (isWaiting) cardClass = "border-blue-200 bg-blue-50 opacity-90";
              if (isReturned) cardClass = "border-orange-300 bg-orange-50 animate-pulse-light";

              return (<div key={tarefa.id} className={`p-4 rounded-xl border-2 transition-all ${cardClass}`}><div className="mb-4">
                  <h4 className={`text-lg font-bold flex items-center gap-2 ${isCompleted || isCanceled ? 'line-through text-slate-400' : 'text-slate-700'}`}>
                    {isReturned && <AlertTriangle className="text-orange-600" size={20} />}
                    {tarefa.template?.title}
                    {isWaiting && <span className="text-[10px] bg-blue-100 text-blue-800 px-2 py-1 rounded-full uppercase">Em Revisão</span>}
                    {isReturned && <span className="text-[10px] bg-orange-100 text-orange-800 px-2 py-1 rounded-full uppercase">Corrigir</span>}
                  </h4>
                  <p className="text-sm text-slate-500 mb-1">{tarefa.template?.description}</p>
                  
                  {isReturned && tarefa.manager_notes && (
                    <div className="mt-3 p-2 bg-orange-100 border border-orange-200 rounded text-xs text-orange-900 italic">
                        <strong>Motivo da Devolução:</strong> {tarefa.manager_notes}
                    </div>
                  )}

                  {isPostponed && tarefa.postponed_to && <p className="text-xs text-amber-700 mt-1">Nova data: {new Date(tarefa.postponed_to).toLocaleDateString('pt-BR')}</p>}
                </div>
                
                {!isCanceled && !isPostponed && !isWaiting && (
                    <div className="flex gap-2 flex-wrap">
                        <button onClick={(e) => handleConcluir(e, tarefa)} className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-lg font-bold transition-colors ${isCompleted ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-green-100 hover:text-green-700'}`}>
                            {isReturned ? <RefreshCcw size={18} /> : <CheckCircle size={18} />} 
                            {isCompleted ? "Feito" : isReturned ? "Refazer e Enviar" : "Concluir"}
                        </button>
                        {!isCompleted && (
                            <>
                                <button onClick={(e) => abrirModalAdiar(e, tarefa)} className="flex items-center justify-center gap-2 py-2 px-4 rounded-lg font-bold bg-slate-100 text-slate-600 hover:bg-amber-100"><Clock size={18} /> Adiar</button>
                                <button onClick={(e) => abrirModalCancelar(e, tarefa)} className="flex items-center justify-center gap-2 py-2 px-4 rounded-lg font-bold bg-slate-100 text-slate-600 hover:bg-red-100"><XCircle size={18} /> Cancelar</button>
                            </>
                        )}
                    </div>
                )}
                ...
                {isWaiting && (
                   <div className="text-xs text-blue-600 font-bold italic flex items-center gap-1">
