import "jsr:@supabase/functions-js/edge-runtime.d.ts";
import { asaasRequest } from "../_shared/asaasClient.ts";
import { CORS, validateAuth, validateOrgScoping, jsonResponse, errorResponse } from "../_shared/authGuard.ts";

Deno.serve(async (req: Request) => {
  if (req.method === "OPTIONS") return new Response(null, { headers: CORS });
  if (req.method !== "POST") return errorResponse("Método não permitido", 405);

  try {
    const { callerProfile, adminClient, asaasApiKey, asaasBaseUrl } = await validateAuth(req);

    let body: {
      organizationId: string;
      subscriptionId: string;
      billingType: string;
      creditCardToken?: string;
      nextDueDate?: string;
    };
    try {
      body = await req.json();
    } catch {
      return errorResponse("Corpo da requisição inválido", 400);
    }

    const { organizationId, subscriptionId, billingType } = body;
    if (!organizationId || !subscriptionId || !billingType) {
      return errorResponse("Campos obrigatórios: organizationId, subscriptionId, billingType", 400);
    }

    const validBillingTypes = ["CREDIT_CARD", "PIX", "BOLETO"];
    if (!validBillingTypes.includes(billingType)) {
      return errorResponse("billingType inválido. Use: CREDIT_CARD, PIX ou BOLETO", 400);
    }

    if (billingType === "CREDIT_CARD" && !body.creditCardToken) {
      return errorResponse("creditCardToken obrigatório para cartão de crédito", 400);
    }

    validateOrgScoping(callerProfile, organizationId, "Organização inválida");

    const [orgRes, subRes] = await Promise.all([
      adminClient.from("organizations")
        .select("id, name, asaas_customer_id")
        .eq("id", organizationId).single(),
      adminClient.from("subscriptions")
        .select("id, max_stores, price_per_store, asaas_subscription_id")
        .eq("id", subscriptionId).single(),
    ]);

    if (!orgRes.data) return errorResponse("Organização não encontrada", 404);
    if (!subRes.data) return errorResponse("Assinatura não encontrada", 404);

    const org = orgRes.data;
    const sub = subRes.data;

    if (!org.asaas_customer_id) {
      return errorResponse("Organização sem cliente Asaas. Crie o cliente primeiro.", 422);
    }

    if (sub.asaas_subscription_id) {
      return jsonResponse({
        asaas_subscription_id: sub.asaas_subscription_id,
        message: "Assinatura Asaas já existe",
      });
    }

    const value = (sub.max_stores || 1) * (sub.price_per_store || 0);

    const asaasPayload: Record<string, unknown> = {
      customer: org.asaas_customer_id,
      billingType,
      value,
      cycle: "MONTHLY",
      description: `Niilu - Plano ${sub.max_stores || 1} loja(s)`,
      externalReference: subscriptionId,
    };

    if (body.nextDueDate) asaasPayload.nextDueDate = body.nextDueDate;
    if (billingType === "CREDIT_CARD" && body.creditCardToken) {
      asaasPayload.creditCardToken = body.creditCardToken;
    }

    const asaasResult = await asaasRequest(
      `${asaasBaseUrl}/subscriptions`,
      "POST",
      asaasApiKey,
      asaasPayload,
    );

    if (!asaasResult.ok) {
      console.error("Asaas subscription error:", JSON.stringify(asaasResult.data));
      const errors = asaasResult.data?.errors as Array<{ description?: string }> | undefined;
      const msg = errors?.[0]?.description || "Erro ao criar assinatura no Asaas";
      return errorResponse(msg, 422);
    }
    const asaasData = asaasResult.data;

    const updatePayload: Record<string, unknown> = {
      asaas_subscription_id: asaasData.id,
      asaas_customer_id: org.asaas_customer_id,
      billing_type: billingType,
      status: "active",
      updated_at: new Date().toISOString(),
    };

    const { error: updateError } = await adminClient
      .from("subscriptions")
      .update(updatePayload)
      .eq("id", subscriptionId);

    if (updateError) console.error("DB update error:", updateError);

    return jsonResponse({
      asaas_subscription_id: asaasData.id,
      asaas_status: asaasData.status,
      value: asaasData.value,
      nextDueDate: asaasData.nextDueDate,
      message: "Assinatura criada com sucesso",
    }, 201);
  } catch (err) {
    if (err instanceof Response) return err;
    console.error("Unexpected error:", err);
    return errorResponse("Um erro ocorreu, por favor tente novamente", 500);
  }
});
